...
//deliveryAdmin.tsx
const TransportAdmin: React.FC = () => {
  const [provinceList] = useState(provinces);

  const [deliveries, setDeliveries] = useState<TransportData[]>([]);
  const [filteredDeliveries, setFilteredDeliveries] = useState<TransportData[]>(
    []
  );
  ...
  const [selectedDelivery, setSelectedDelivery] =
    useState<TransportData | null>(null);
  ...
  const handleDelete = async (delivery: TransportData) => {
    try {
      if (
        window.confirm(
  ...
  const handleViewDetails = (delivery: TransportData) => {
  ...
  const filterAndSortDeliveries = (
    ...
    data.sort((a, b) => {
      ...
      if (field === "uploadTime") {
        ...
        valueA = a[field as keyof TransportData] as string | number | undefined;
        valueB = b[field as keyof TransportData] as string | number | undefined;
      }
    ...
    const fetchDeliveries = async () => {
      try {
        ...
        const deliveryData = querySnapshot.docs.map((doc) => {
          const docData = doc.data() as TransportData;
  ...
  return (
    <div className="form-container mx-auto mt-1">
      ...
      <h1 className="text-success text-center mb-3">
        แดชบอร์ดแอดมินสำหรับข้อมูลการส่งรถส่งของกลับบ้าน หมา แมว สัตว์เลี้ยง
      </h1>
